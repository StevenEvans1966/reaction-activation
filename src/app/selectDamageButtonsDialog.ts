...
import { NameAndIconData, getUnknownDamageTypes as getUnknownDamageTypes, getAllDamageTypeNameAndIcon, getDamageTypeNameAndIcon } from "../module/config.js"

export type DialogRtn = { damageType: string, nameAndIcon: NameAndIconData }

export async function showSelectDamageButtonsDialog(damageTypes: string[], defaultValue: string, flavor?: string): Promise<DialogRtn | undefined> {
  ...
  return new Promise<DialogRtn>((resolve, reject) => {
...
class SelectDamageButtonsDialog extends Application {
  ...
  async getData(options) {
    this.data.buttons = this.data.items.reduce((acc: {}, item: NameAndIconData) => {
      acc[randomID()] = {
  ...
  _onKeyDown(event) {
    ...
    if (event.key === "Escape" || event.key === "Enter") {
      ...
      if (this.data.close) this.data.close({ name: "keydown" });
      this.close();
    }
  }
