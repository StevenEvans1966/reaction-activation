import { getDamageTypeForReaction, addReactionActivation, removeReactionActivation, getReactionActivation } from "./module/reactionActivation.js";
...
const reaction_filter_midi_qol_effect = "flags.midi-qol.reactionHooks";
...
async function applyActiveEffect(actor, change) {
	if (change.key !== reaction_filter_midi_qol_effect) {
		return;
	}
	let data;
	try {
		data = JSON.parse(change.value);
	}
	catch (error) {
		data = undefined;
...
function reactionFilter(reactions, options) {
	var i = 0;
	while (i < reactions.length) {
		...
		const itemResults = options.workflowOptions.reactionChecks?.get(itemUuid);
